...
export async function POST(request: Request) {
  ...
  try {
    ...
    const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY }); const stream = await client.messages.stream({ model: 'claude-sonnet-4-6', max_tokens: 1200, system: 'Answer ONLY using the provided notes. If they do not contain the answer, say so explicitly. Cite note titles inline.', messages: [{ role: 'user', content: `Question: ${question}\n\nNotes:\n${context || '(No notes found)'}` }] });
    const encoder = new TextEncoder(); const body = new ReadableStream({ async start(controller) { controller.enqueue(encoder.encode(JSON.stringify({ sources }) + '\n')); for await (const event of stream) if (event.type === 'content_block_delta' && event.delta.type === 'text_delta') controller.enqueue(encoder.encode(event.delta.text)); controller.close(); } });
